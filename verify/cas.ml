...
let vars_in_lex_order es =
  ...
  |> List.sort cmp_var_name_sidx

let vars_in_order ord es =
  match ord with
...
let polys_of_espec ?(ord=alg_option.cas_variable_order) vgen s =
  ...
  let rec convert generator_ps post =
    ...
    | Eeq (e1, e2) ->
       ...
       let vars = vars_in_order ord (rcons ideal p) in
       [(post, vars, ideal, p)]
    | Eeqmod (e1, e2, ms) ->
...
(**
   ...
   @return a list of entailments [(post1, vars1, ideal1, p1); ...; (postn, varsn, idealn, pn)]
           ...
           order [varsi]
 *)
let polys_of_espec_two_phase ?(ord=alg_option.cas_variable_order) ?(sliced=false) vgen s =
      ...
      List.rev (List.rev_map
                  (fun (post, sc, p, ms) ->
                    ...
                    | [] -> (* skip rewriting in this case *)
                       ...
                       (post, vars_in_order ord (rcons sliced_ideal p), sliced_ideal, p)
                    | _ -> let (rewritten_ideal_aps, post_p_ms_list) = do_rewriting (List.rev_append (List.rev_map add_dummy_annot (List.rev ms)) sliced_ideal_aps) [(post, sc, p, ms)] ms in
                           (match post_p_ms_list with
                            | (post, _, p, _)::[] -> let rewritten_ideal = remove_annots rewritten_ideal_aps in
                                                     (post, vars_in_order ord (rcons rewritten_ideal p), rewritten_ideal, p)
...
let bprint_singular_input ?(ord=alg_option.cas_monomial_order) ?comments buf vars gen p =
  let bprint_varseq () =
    match vars with
  ...
  let mon_ord = get_mon_ord ord Singular in
  match gen with
  | [] ->
...
let generate_singular_input ?(ord=alg_option.cas_monomial_order) ?comments vars gen p =
  let varseq =
    match vars with
    | [] -> "x"
...
let bprint_sage_input ?(ord=alg_option.cas_monomial_order) ?comments buf vars gen p =
  ...
  let mon_ord = get_mon_ord ord Sage in
  match gen with
  | [] ->
    (* If gen is empty, we simply check if p equals 0. *)
...
let generate_sage_input ?(ord=alg_option.cas_monomial_order) ?comments vars gen p =
  let varseq =
    match vars with
    | [] -> "x"
    | _ -> String.concat "," (tmap (fun v -> v.cached_name) vars) in
...
let bprint_magma_input ?(ord=alg_option.cas_monomial_order) ?comments buf vars gen p =
  let bprint_varseq buf =
    match vars with
    | [] -> Buffer.add_char buf 'x'
    | _ -> bprint_list buf "," (fun buf v -> Buffer.add_string buf v.cached_name) vars in
  ...
  let mon_ord = get_mon_ord ord Magma in
  match gen with
  | [] ->
...
let generate_magma_input ?(ord=alg_option.cas_monomial_order) ?comments vars gen p =
  let varseq =
    match vars with
    | [] -> "x"
    | _ -> String.concat "," (tmap (fun v -> v.cached_name) vars) in
...
let bprint_mathematica_input ?(ord=alg_option.cas_monomial_order) ?comments buf vars gen p =
  let bprint_varseq buf =
    match vars with
    | [] -> Buffer.add_char buf 'x'
  ...
  let mon_ord = get_mon_ord ord Mathematica in
  match gen with
  | [] ->
    (* If gen is empty, we simply check if p equals 0. (not tested) *)
    bprint_comment buf; Buffer.add_char buf '\n';
...
let generate_mathematica_input ?(ord=alg_option.cas_monomial_order) ?comments vars gen p =
  let varseq =
    match vars with
    | [] -> "x"
...
let bprint_macaulay2_input ?(ord=alg_option.cas_monomial_order) ?comments buf vars gen p =
  ...
  let mon_ord = get_mon_ord ord Macaulay2 in
  match gen with
  | [] ->
    (* If gen is empty, we simply check if p equals 0. (not tested) *)
    bprint_comment buf; Buffer.add_char buf '\n';
...
let generate_macaulay2_input ?(ord=alg_option.cas_monomial_order) ?comments vars gen p =
  let (vars, gen, p, default_generator) =
    let dummy_var =
      mkvar
        ~newvid:true
...
let bprint_maple_input ?(ord=alg_option.cas_monomial_order) ?comments buf vars gen p =
  let const_gen =
  ...
  let mon_ord = get_mon_ord ord Maple in
  match gen with
  | [] ->
    (* If gen is empty, we simply check if p equals 0. (not tested) *)
    bprint_comment buf;
...
let generate_maple_input ?(ord=alg_option.cas_monomial_order) ?comments vars gen p =
  let const_gen () =
    let (const_gen, poly_gen) = List.partition is_eexp_over_const gen in
    let _ =
      if List.length poly_gen > 0
...
let bprint_maxima_input ?(ord=alg_option.cas_monomial_order) ?comments buf vars gen p =
  ...
  let mon_ord = get_mon_ord ord Maxima in
  match gen with
  | [] ->
    (* If gen is empty, we simply check if p equals 0. (not tested) *)
    bprint_comment buf;
