...
module Fnumber: FloatType with type t = Mpfrf.t = struct
  ...
  let cmp x y =
    match Mpfrf.nan_p x, Mpfrf.nan_p y with
    | true, true -> 0
    | false, false -> Mpfrf.cmp x y
    | _ -> raise (Invalid_argument "Fnumber.cmp: NaN is unordered with a non-NaN value")
  let cmp_int x n =
    if Mpfrf.nan_p x then raise (Invalid_argument "Fnumber.cmp_int: NaN is unordered with an integer")
    else Mpfrf.cmp_int x n
  let eq x y =
    match Mpfrf.nan_p x, Mpfrf.nan_p y with
    | true, true -> true
    | false, false -> Mpfrf.cmp x y = 0
    | _ -> false
  let eq_int x n =
    if Mpfrf.nan_p x then raise (Invalid_argument "Fnumber.eq_int: NaN is unordered with an integer")
    else Mpfrf.cmp_int x n = 0
  let is_representable p f = (* this func is generated by ChatGPT cuz I can't be bothered to to write it myself *)
  ...
  let round_to p ~rnd f = (* this func is generated by ChatGPT cuz I can't be bothered to write it myself *)
...
module Qnumber: FloatType with type t = Mpqf.t = struct
  ...
  let eq x y = Mpqf.cmp x y = 0
  let eq_int x n = Mpqf.cmp_int x n = 0
  let is_representable p q = (* this func is generated by ChatGPT cuz I can't be bothered to write it myself *)
    let {precision; emax; emin} = get_mpfr_fmt p in
    let old_emin = Mpfr.get_emin () in
    let old_emax = Mpfr.get_emax () in
    let _ = Mpfr.set_emin emin in
    ...
    with e ->
      ...
      let _ = Mpfr.set_emax old_emax in
      raise e
  let round_to p ~rnd q = (* this func is generated by ChatGPT cuz I can't be bothered to write it myself *)
    let { precision; emin; emax } = get_mpfr_fmt p in
    let old_emin = Mpfr.get_emin () in
    let old_emax = Mpfr.get_emax () in
    let _ = Mpfr.set_emin emin in
