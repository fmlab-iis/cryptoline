{
  ...
  zetas_inv[255] = zetas_inv[255] / 256 % q;

  k = 1;
  zetas_exp = vector(752);
  for(level = 0, 7,
    if(level <= 3, e = 16, e = 128/2^level);
    for(i = 1, 2^level,
      for(j = 1, e,
        zetas_exp[k] = zetas[2^level + i];
        k++;
      );
    );
  );
  ...
  k = 1;
  zetas_inv_exp = vector(752);
  for(level = 0, 7,
    if(level >= 4, e = 16, e = 2^level);
    for(i = 1, 2^(7-level),
      for(j = 1, e,
        zetas_inv_exp[k] = zetas_inv[256 - 2^(8-level) + i];
        k++;
      );
    );
  );
