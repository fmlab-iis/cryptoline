...
# The following line marks the end of levels 0t1 (and the start of levels 2t7)
...
def gen_ins():
    str = joinN([f"mov L0x{INS_ADDR + i * OFFSET:12x} {INS_VAR_NAME}{i:03d};" for i in range(INS_NUM)], 4, " ", "\n")
    print("(* INPUTS *)")
    print(str)
...
def gen_poly():
    strs = ["(* INP_POLY *)",
            ...
            f"  {POLY_NAME}**2 = poly {POLY_VAR} [",
            "    " + joinN([f"{INS_VAR_NAME}{i:03d}" for i in range(INS_NUM)], 8, ", ", ",\n    "),
            "  ] &&& true;"
        ]
    print("\n".join(strs))
...
def print_cut_level0t1():
    moduli = ntt_poly(1)
    expn = 64
    level = 1
    print("cut")
    print("  and [")
    print(",\n".join([f"    eqmod (inp_poly**2) ({poly(expn, i)}) [{Q}, X**{expn} - ({moduli[i]})]" for i in range(len(moduli))]))
    print("  ]")
    print("  &&")
    print("  and [")
    rngs = joinN([f"(-{Q*(level+2)})@{BITS} <s L0x{INS_ADDR + i * OFFSET:12x}, L0x{INS_ADDR + i * OFFSET:12x} <s ({Q*(level+2)})@{BITS}" for i in range(INS_NUM)], 4, ", ", ",\n    ")
    print("    " + rngs)
    print("  ];")

def gen_post(block_finals):
    moduli = ntt_poly(7)
    print("{")
    print("  and [")
    print(",\n".join([f"    eqmod (inp_poly**2) L0x{INS_ADDR + OFFSET*(i%8*8 + i//8 + off*64):12x} [{Q}, X - ({moduli[i + 64 * off]})]" for off in range(4) for i in range(64)]))
    print("  ]")
    print("  &&&")
    print("  and [")
    rngs = joinN([f"(-{Q*(LEVELS+1)})@{BITS} <s L0x{INS_ADDR + i * OFFSET:12x}, L0x{INS_ADDR + i * OFFSET:12x} <s ({Q*(LEVELS+1)})@{BITS}" for i in range(INS_NUM)], 4, ", ", ",\n    ")
    print("    " + rngs)
    prove_with = ", ".join([str(i) for i in block_finals])
    print(f"  ] prove with [cuts [{prove_with}]]")
    print("}")

def gen_rcut(level, ymms):
    print("rcut")
    print("  and [")
    negs = "[" + ", ".join([f"(-{Q*(level+2)})@{BITS}" for i in range(8)]) + "]"
    poss = "[" + ", ".join([f"({Q*(level+2)})@{BITS}" for i in range(8)]) + "]"
    rngs = joinN([f"{negs} <s {ymm}, {ymm} <s {poss}" for ymm in ymms], 2, ", ", ",\n    ")
    print("    " + rngs)
    prove_with = " prove with [cuts [0]]" if level == 2 else ""
    print(f"  ]{prove_with};")

ymms = [
    [],                                                                         # level 0
    [],                                                                         # level 1
    ["%ymm3", "%ymm4", "%ymm5", "%ymm6", "%ymm8", "%ymm9", "%ymm10", "%ymm11"], # level 2
    ["%ymm7", "%ymm3", "%ymm8", "%ymm4", "%ymm5", "%ymm10", "%ymm6", "%ymm11"], # level 3
    ["%ymm9", "%ymm8", "%ymm7", "%ymm6", "%ymm5", "%ymm4", "%ymm3", "%ymm11"],  # level 4
    ["%ymm9", "%ymm8", "%ymm7", "%ymm6", "%ymm5", "%ymm4", "%ymm3", "%ymm11"],  # level 5
    ["%ymm9", "%ymm8", "%ymm7", "%ymm6", "%ymm5", "%ymm4", "%ymm3", "%ymm11"],  # level 6
    ["%ymm9", "%ymm8", "%ymm7", "%ymm6", "%ymm5", "%ymm4", "%ymm3", "%ymm11"]   # level 7
    ]

def gen_block_rcut(block):
    print("rcut")
    print("  and [")
    rngs = joinN([f"(-{Q*(LEVELS+1)})@{BITS} <s L0x{INS_ADDR + (block * 64 + i) * OFFSET:12x}, L0x{INS_ADDR + (block * 64 + i) * OFFSET:12x} <s ({Q*(LEVELS+1)})@{BITS}" for i in range(64)], 4, ", ", ",\n    ")
    print("    " + rngs)
    print("  ];")
...
def main():
    ...
    gen_poly()
    rcut = 0
    print(f"# ----- Cut Levels 0t1, rcut #{rcut} -----")
    # Insert cut after:
    # (* vmovdqa %ymm11,0x3e0(%rdi)                      #! EA = L0x7fffffff48a0; PC = 0x5555555736b9 *)
    # mov [L0x7fffffff48a0, L0x7fffffff48a4, L0x7fffffff48a8, L0x7fffffff48ac, L0x7fffffff48b0, L0x7fffffff48b4, L0x7fffffff48b8, L0x7fffffff48bc] %ymm11;
    print_cut_level0t1()
    rcut = rcut + 1
    # Insert rcut before loading ZETA such as:
    # vmovdqa		(_ZETAS_QINV+8+8*\off)*4(%rsi),%ymm1
    block_finals = []
    for block in range(4):
        for i in range(2, 7):
            print(f"# ----- Cut Block {block}, Cut Level {i}, rcut #{rcut} -----")
            gen_rcut(i, ymms[i])
            rcut = rcut + 1
        print(f"# ----- Cut Block {block}, Cut Level 7, rcut #{rcut} -----")
        gen_block_rcut(block)
        block_finals.append(rcut)
        rcut = rcut + 1
    print("# ----- Postcondition -----")
    gen_post(block_finals)
