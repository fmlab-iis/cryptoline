...
def gen_proc():
    ...
    print(f"  {RNG_TRACK}:")
    print("  and [")
    rngs = joinN([f"(-{Q})@{BITS} < {INS_VAR_NAME}{i:03d}, {INS_VAR_NAME}{i:03d} < ({Q})@{BITS}" for i in range(INS_NUM)], 4, ", ", ",\n    ")
    print("    " + rngs)
    print("  ]")
    print("  &&&")
    print("  true")
...
def gen_poly():
    moduli = ntt_poly(7)
    eqs = ",\n".join([f"    eqmod ({POLY_NAME}**2) {INS_VAR_NAME}{off*64 + i:03d} [{Q}, X - ({moduli[i + 64 * off]})]" for off in range(4) for i in range(64)])
    strs = ["(* INP_POLY *)",
            f"ghost {POLY_VAR}@bit, {POLY_NAME}@bit :",
            "  and [",
            f"{eqs}",
            "  ] &&& true;"
        ]
...
def print_cut_level0t4(block, level, ymms, lvrngs):
    ...
    moduli = ntt_poly(7 - level - 1)
    vsize = len(ymms)              # number of ymm vectors
    npoly = 256 // (2**(level + 1)) // 4 # number of polynomials in this block
    ...
    if level == 0:
        ...
        # poly[1] = ymms[1][0] ymms[5][0]
        # poly[2] = ymms[2][0] ymms[6][0]
        # poly[3] = ymms[3][0] ymms[7][0]
        # poly[4] = ymms[0][1] ymms[4][1]
        # ...
        # poly[31] = ymms[3][7] ymms[7][7]
        for i in range(npoly):
            coefs.append([f"{ymms[i % 4]:>6s}[{i // 4}]", f"{ymms[(i % 4) + 4]:>6s}[{i // 4}]"])
        for i in range(len(ymms)):
            start = [0, 2, 4, 6, 1, 3, 5, 7][i]
            rngs.append([lvrngs[level][block * 64 + start + j * 8] for j in range(8)])
    elif level == 1:
        # poly[0] = ymms[0][0] ymms[1][0] ymms[4][0] ymms[5][0]
        # poly[1] = ymms[2][0] ymms[3][0] ymms[6][0] ymms[7][0]
        # poly[2] = ymms[0][1] ymms[1][1] ymms[4][1] ymms[5][1]
        # poly[3] = ymms[2][1] ymms[3][1] ymms[6][1] ymms[7][1]
        # ...
        # poly[15] = ymms[2][7] ymms[3][7] ymms[6][7] ymms[7][7]
        for i in range(npoly):
            coefs.append([f"{ymms[(i % 2) * 2]:>6s}[{i // 2}]", f"{ymms[(i % 2) * 2 + 1]:>6s}[{i // 2}]", f"{ymms[(i % 2) * 2 + 4]:>6s}[{i // 2}]", f"{ymms[(i % 2) * 2 + 5]:>6s}[{i // 2}]"])
        for i in range(len(ymms)):
            start = [0, 1, 4, 5, 2, 3, 6, 7][i]
            rngs.append([lvrngs[level][block * 64 + start + j * 8] for j in range(8)])
    elif level == 2:
        # poly[0] = ymms[0][0] ymms[1][0] ymms[2][0] ymms[3][0] ymms[4][0] ymms[5][0] ymms[6][0] ymms[7][0]
        # poly[1] = ymms[0][1] ymms[1][1] ymms[2][1] ymms[3][1] ymms[4][1] ymms[5][1] ymms[6][1] ymms[7][1]
        # ...
        # poly[7] = ymms[0][7] ymms[1][7] ymms[2][7] ymms[3][7] ymms[4][7] ymms[5][7] ymms[6][7] ymms[7][7]
        for i in range(npoly):
            coefs.append([f"{ymm}[{i}]" for ymm in ymms])
        for i in range(len(ymms)):
            start = i
            rngs.append([lvrngs[level][block * 64 + start + j * 8] for j in range(8)])
    elif level == 3:
        ...
        # ...
        for i in range(npoly):
            coefs.append([f"{ymms[0]}[{i * 2}]", f"{ymms[0]}[{i * 2 + 1}]",
                          f"{ymms[1]}[{i * 2}]", f"{ymms[1]}[{i * 2 + 1}]",
                          f"{ymms[2]}[{i * 2}]", f"{ymms[2]}[{i * 2 + 1}]",
                          f"{ymms[3]}[{i * 2}]", f"{ymms[3]}[{i * 2 + 1}]",
                          f"{ymms[4]}[{i * 2}]", f"{ymms[4]}[{i * 2 + 1}]",
                          f"{ymms[5]}[{i * 2}]", f"{ymms[5]}[{i * 2 + 1}]",
                          f"{ymms[6]}[{i * 2}]", f"{ymms[6]}[{i * 2 + 1}]",
                          f"{ymms[7]}[{i * 2}]", f"{ymms[7]}[{i * 2 + 1}]"
                        ])
        for i in range(len(ymms)):
            start = i * 2
            rngs.append([lvrngs[level][block * 64 + start + (j // 2) * 16 + j % 2] for j in range(8)])
    elif level == 4:
        # poly[0] = ymms[0][0:4] ymms[1][0:4] ymms[2][0:4] ymms[3][0:4] ymms[4][0:4] ymms[5][0:4] ymms[6][0:4] ymms[7][0:4]
        # ...
        for i in range(npoly):
            coefs.append([f"{ymms[0]}[{i * 4}]", f"{ymms[0]}[{i * 4 + 1}]", f"{ymms[0]}[{i * 4 + 2}]", f"{ymms[0]}[{i * 4 + 3}]",
                          f"{ymms[1]}[{i * 4}]", f"{ymms[1]}[{i * 4 + 1}]", f"{ymms[1]}[{i * 4 + 2}]", f"{ymms[1]}[{i * 4 + 3}]",
                          f"{ymms[2]}[{i * 4}]", f"{ymms[2]}[{i * 4 + 1}]", f"{ymms[2]}[{i * 4 + 2}]", f"{ymms[2]}[{i * 4 + 3}]",
                          f"{ymms[3]}[{i * 4}]", f"{ymms[3]}[{i * 4 + 1}]", f"{ymms[3]}[{i * 4 + 2}]", f"{ymms[3]}[{i * 4 + 3}]",
                          f"{ymms[4]}[{i * 4}]", f"{ymms[4]}[{i * 4 + 1}]", f"{ymms[4]}[{i * 4 + 2}]", f"{ymms[4]}[{i * 4 + 3}]",
                          f"{ymms[5]}[{i * 4}]", f"{ymms[5]}[{i * 4 + 1}]", f"{ymms[5]}[{i * 4 + 2}]", f"{ymms[5]}[{i * 4 + 3}]",
                          f"{ymms[6]}[{i * 4}]", f"{ymms[6]}[{i * 4 + 1}]", f"{ymms[6]}[{i * 4 + 2}]", f"{ymms[6]}[{i * 4 + 3}]",
                          f"{ymms[7]}[{i * 4}]", f"{ymms[7]}[{i * 4 + 1}]", f"{ymms[7]}[{i * 4 + 2}]", f"{ymms[7]}[{i * 4 + 3}]"
                        ])
        for i in range(len(ymms)):
            start = i * 4
            rngs.append([lvrngs[level][block * 64 + start + (j // 4) * 32 + j % 4] for j in range(8)])
    print("cut")
    print("  and [")
    estr = ",\n    ".join([f"eqmod ({2**(level+1)}*{POLY_NAME}**2) ({mk_poly(coefs[i])}) [{Q}, {POLY_VAR}**{2**(level + 1)} - ({moduli[i + block * npoly]:8d})]" for i in range(npoly)])
    print("    " + estr)
    eprove_with = " prove with [precondition, all ghosts]" if block != 0 and level == 0 else ""
    print(f"  ]{eprove_with} .")
    print(f"  {RNG_TRACK}:")
    print("  and [")
    pos = lambda i : ', '.join([f"({r})" for r in rngs[i]])
    neg = lambda i : ', '.join([f"({-r})" for r in rngs[i]])
    rstr = joinN([f"[{neg(i)}] < {ymms[i]}, {ymms[i]} < [{pos(i)}]" for i in range(len(ymms))], 2, ", ", ",\n    ")
    print("    " + rstr)
    rprove_with_precondition = ", precondition" if block != 0 and level == 0 else ""
    rprove_with = f" prove with [algebra solver {ALG_SOLVER}{rprove_with_precondition}]"
    print(f"  ]{rprove_with}")
    print("  &&&")
    print("  true;")

def print_cut_level5(block, lvrngs):
    num_blocks = 4
    factor = 7
    num = 64
    level = 5
    ...
    print("cut")
    print("  and [")
    print(f"    eqmod ({2**(level+1)}*{POLY_NAME}**2) ({out_poly(num, block)}) [{Q}, {POLY_VAR}**{64} - ({moduli[block]:8d})]")
    print("  ] .")
    print(f"  {RNG_TRACK}:")
    print("  and [")
    rng_f = lambda i : lvrngs[level][block * num + i]
    rngs = joinN([f"(-{rng_f(i)}) < L0x{INS_ADDR + (block * num + i) * OFFSET:12x}, L0x{INS_ADDR + (block * num + i) * OFFSET:12x} < ({rng_f(i)})" for i in range(num)], 4, ", ", ",\n    ")
    print("    " + rngs)
    print(f"  ] prove with [algebra solver {ALG_SOLVER}]")
    print("  &&&")
    print("  true;")
...
def print_rcut_level6(block, lvrngs, rcuts_level5):
    ...
    print(f"  ] prove with [algebra solver {ALG_SOLVER}, cuts [{rcuts}]];")

def print_rcut_level7(block, lvrngs):
    print("ecut")
    print(f"  {RNG_TRACK}:")
    print("  and [")
    rngs = joinN([f"(-{Q}) < L0x{INS_ADDR + (i * 32 + block * 8 + j) * OFFSET:12x}, L0x{INS_ADDR + (i * 32 + block * 8 + j) * OFFSET:12x} < ({Q})" for i in range(8) for j in range(8)], 4, ", ", ",\n    ")
    print("    " + rngs)
    print(f"  ] prove with [algebra solver {ALG_SOLVER}];")
...
def gen_post(eblock_finals, rblock_finals):
    ...
    print("{")
    print("  and [")
    print(f"    eqmod ((2**32)*({POLY_NAME}**2)) ({poly}) [{Q}, {POLY_VAR}**{N} + 1]")
    eprove_with = ", ".join([str(i) for i in eblock_finals[0:-1]])
    print(f"  ] prove with [precondition, all ghosts, cuts [{eprove_with}]] .")
    print(f"  {RNG_TRACK}:")
    print("  and [")
    rngs = joinN([f"(-{Q}) < L0x{INS_ADDR + i * OFFSET:12x}, L0x{INS_ADDR + i * OFFSET:12x} < ({Q})" for i in range(INS_NUM)], 4, ", ", ",\n    ")
    print("    " + rngs)
    rprove_with = ", ".join([str(i) for i in rblock_finals[0:-1]])
    print(f"  ] prove with [algebra solver {ALG_SOLVER}, cuts [{rprove_with}]]")
    print("  &&&")
    print("  true")
    print("}")
...
def main():
    ...
    for block in range(4):
        for level in range(5):
            ...
            print_cut_level0t4(block, level, ymms[level], lvrngs)
            ecut = ecut + 1
            rcut = rcut + 1
        print(f"# ----- Cut Block {block}, iNTT Level 5, ecut #{ecut}, rcut #{rcut} -----")
        print_cut_level5(block, lvrngs)
