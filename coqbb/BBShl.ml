...
| O -> ((g, []), ls)
| S m ->
  let (p, ls_m) = bit_blast_shl_int g ls m in
  let (g_m, cs_m) = p in
  let (p0, ls1) = bit_blast_shl_int1 g_m ls_m in
...
let rec bit_blast_shl_rec g ls ns i =
  ...
  | [] -> ((g, []), ls)
  | ns_hd :: ns_tl ->
    let (p, ls_tl) = bit_blast_shl_rec g ls ns_tl (muln (S (S O)) i) in
    let (g_tl, cs_tl) = p in
    let (p0, ls_hd) = bit_blast_shl_int g_tl ls_tl i in
